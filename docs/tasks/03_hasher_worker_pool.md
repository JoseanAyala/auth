# Task 03 — Build the Hasher: worker pool with Argon2 hashing
...
- [ ] `internal/hasher/` package exists with `dispatcher.go` and `worker.go`.
...
- [ ] Tests pass under `go test -race ./internal/hasher/...`.
